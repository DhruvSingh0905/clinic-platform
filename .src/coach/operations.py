"""Typed operation set for Coach Platform.

The closed, enumerated set of operations any actor can perform.
CRITICAL: No coach-side substance-write operation exists in this set.
The coach can view substance data read-only; the user self-logs.
...
# --- Coach operations (the lawful coaching surface) ---

@dataclass
class SetTrainingBlock:
    """Coach sets a training block for an athlete."""
    athlete_id: str
    name: str
    block_type: str  # hypertrophy, strength, deload, prep, offseason
    start_date: str
...
class EndTrainingBlock:
    """Coach ends an active training block."""
    athlete_id: str
    block_id: str
    end_date: str
...
class SetNutritionTarget:
    """Coach sets macro/calorie targets."""
    athlete_id: str
    calories: int
    protein_g: int
    carbs_g: int
...
class AddRecoveryNote:
    """Coach adds a recovery note/directive."""
    athlete_id: str
    note_type: str  # sleep, deload, rest_day, active_recovery, note
    content: str
...
class NudgeSensitivity:
    """Coach adjusts detector sensitivity for a client (within guardrails)."""
    athlete_id: str
    detector_id: str
...
# --- User operations (user self-logs, never coach) ---

@dataclass
class UserLogSubstanceEvent:
    """User logs their own substance event. Coach CANNOT invoke this."""
    athlete_id: str
    compound_name: str
    compound_class: str
...
# The complete operation set — coach operations + user operations
COACH_OPERATIONS = {
...
# ALL operations — note: no overlap, no coach substance ops
ALL_OPERATIONS = {**COACH_OPERATIONS, **USER_OPERATIONS}
...
def commit_operation(conn, actor_id: str, actor_role: str, operation) -> dict:
    ...
    Coach cannot invoke USER_LOG_SUBSTANCE_EVENT.
    """
    op_type = type(operation).__name__

    # Enforce the substance boundary
    if actor_role == "coach" and isinstance(operation, UserLogSubstanceEvent):
        raise PermissionError(
            "Coach cannot log substance events. "
    ...
    if isinstance(operation, SetTrainingBlock):
        ...
        conn.execute(
            "INSERT INTO training_block (id, athlete_id, coach_id, name, block_type, start_date, end_date, notes) VALUES (?, ?, ?, ?, ?, ?, ?, ?)",
            (block_id, operation.athlete_id, actor_id, operation.name, operation.block_type, operation.start_date, operation.end_date, operation.notes)
        )
    elif isinstance(operation, EndTrainingBlock):
        conn.execute(
            "UPDATE training_block SET end_date = ?, status = 'completed' WHERE id = ? AND athlete_id = ?",
            (operation.end_date, operation.block_id, operation.athlete_id)
        )
        ...
        conn.execute(
            ...
            (target_id, operation.athlete_id, actor_id, operation.calories, operation.protein_g, operation.carbs_g, operation.fat_g, operation.notes, operation.effective_date)
        )
    elif isinstance(operation, AddRecoveryNote):
        note_id = str(uuid.uuid4())
        conn.execute(
            "INSERT INTO recovery_note (id, athlete_id, coach_id, note_type, content) VALUES (?, ?, ?, ?, ?)",
            (note_id, operation.athlete_id, actor_id, operation.note_type, operation.content)
        )
        ...
        conn.execute(
            ...
            (operation.athlete_id, compound_id, operation.event_type, operation.dose_mg, operation.frequency, operation.route, datetime.now().isoformat())
        )
