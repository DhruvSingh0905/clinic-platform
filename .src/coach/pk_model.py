...
def _generate_for_compound(
    ...
    while current <= end_date:
        ...
        # After dose reduction (blast→cruise), levels can exceed 1.0 as
        # residual blast-dose compound decays toward new steady state
        normalized = min(total_level / steady_state_peak, 5.0)  # cap at 5x for blast→cruise
...
def _estimate_steady_state_peak(
    ...
    This means after a dose change (blast→cruise), the new steady state
    becomes 100% and residual blast levels show >100%, decaying naturally.
    """
    if not doses:
        return 1.0
