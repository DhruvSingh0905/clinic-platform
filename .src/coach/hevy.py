...
Athlete trains in Hevy → workouts auto-sync here → stall detector runs →
coach sees findings in roster → coach adjusts routine → pushes back to Hevy →
athlete gets notified.
"""
import sqlite3
import json
import uuid
...
def get_hevy_api_key(conn: sqlite3.Connection, user_id: str) -> str | None:
    """Get stored Hevy API key for an athlete."""
    row = conn.execute(
        "SELECT last_sync FROM integration_status WHERE athlete_id = ? AND provider = 'hevy'",
        (user_id,)
    ).fetchone()
    ...
    meta = conn.execute(
        "SELECT status FROM integration_status WHERE athlete_id = ? AND provider = 'hevy'",
        (user_id,)
    ).fetchone()
...
# ═══════════════════════════════════════════════════════════════
# Workout sync (Hevy → Coach Platform)
# ═══════════════════════════════════════════════════════════════
...
def sync_workouts_from_hevy(conn: sqlite3.Connection, user_id: str, api_key: str) -> int:
    ...
    conn.execute(
        "UPDATE integration_status SET last_sync = ? WHERE athlete_id = ? AND provider = 'hevy'",
...
def detect_training_stall(conn: sqlite3.Connection, user_id: str, as_of: str | None = None) -> dict | None:
    ...
    phase_row = conn.execute(
        "SELECT phase FROM cycle_phase WHERE user_id = ? AND ended_at IS NULL ORDER BY started_at DESC LIMIT 1",
        (user_id,)
    ).fetchone()
    ...
    for ex in frequent:
        ...
        if len(weights) >= 2:
            peak = max(weights[1:])
            if peak > 0 and (peak - latest) / peak >= 0.05 and phase not in ("prep", "off"):
                drops.append({
                    "exercise": ex["exercise_name"],
                    "current": latest,
                    "peak": peak,
    ...
    # Severity based on phase + health context
    severity = "info"
    ...
    """, (user_id, severity,
          f"{len(stalls)} stall(s) and {len(drops)} drop(s) detected across compound lifts. Phase: {phase}.",
          detail, cutoff, as_of))
    conn.commit()
...
# ═══════════════════════════════════════════════════════════════
# Routine push (Coach Platform → Hevy)
# ═══════════════════════════════════════════════════════════════
...
def push_routine_to_hevy(
    ...
    """Push a routine to the athlete's Hevy account.
    ...
    # Record the push
    conn.execute("""
        INSERT INTO routine_push (coach_id, athlete_id, hevy_routine_id, title, routine_json)
        VALUES (?, ?, ?, ?, ?)
    """, (coach_id, athlete_id, hevy_routine_id, title, routine_json))
    conn.commit()

    # Notify athlete
    create_notification(conn, athlete_id, coach_id, "coach",
                        "routine_created", f"Your coach created {title}",
...
def update_routine_in_hevy(
    ...
    """Update an existing routine in the athlete's Hevy."""
    ...
    conn.execute("""
        INSERT INTO routine_push (coach_id, athlete_id, hevy_routine_id, title, routine_json)
        VALUES (?, ?, ?, ?, ?)
    """, (coach_id, athlete_id, hevy_routine_id, title, new_json))
    conn.commit()

    # Notify athlete with diff
    diff_text = "\n".join(diff_lines) if diff_lines else "Minor adjustments"
    create_notification(conn, athlete_id, coach_id, "coach",
                        "routine_updated", f"Your coach updated {title}",
...
def seed_mock_workouts(conn: sqlite3.Connection, user_id: str):
    ...
    # Marcus (athlete-001): Push/Pull/Legs, bench stalling
    workouts = [
        {
