"""Chat engine for Clinic Platform — role-scoped LLM interaction.

Wraps the copied CDE LLM engine with:
- Two tool sets: patient (all 20 CDE tools) and clinician (read + clinical-surface)
- Substance boundary enforcement via tool set absence
- Multi-tenant briefings scoped by patient_id (passed as user_id to CDE functions)
"""
import sqlite3
import anthropic

from clinic.config import ANTHROPIC_API_KEY, CDE_MODEL
from clinic.llm.tools import TOOL_DEFINITIONS, execute_tool
from clinic.llm.context import build_minimal_briefing
from clinic.operations import (
    SetTrainingBlock, EndTrainingBlock, SetNutritionTarget,
    AddRecoveryNote, commit_operation,
)

# Clinician has substance-write access — patient gets notified
# All CDE tools available to clinician
CLINICIAN_CDE_TOOLS = list(TOOL_DEFINITIONS)

CLINICAL_SURFACE_TOOLS = [
    {
        ...
        "input_schema": {
            ...
            "properties": {
                ...
                "block_type": {"type": "string", "enum": ["exercise", "rehab", "maintenance", "custom"]},
    ...
    },
    {
        ...
        "input_schema": {
            "type": "object",
            "properties": {
                "note_type": {"type": "string", "enum": ["assessment", "plan", "subjective", "objective", "follow_up", "general"]},
                "content": {"type": "string"},
            },
...
CHECKIN_TOOLS = [
    {
        ...
        "description": "Schedule a check-in meeting with the patient. Syncs to Google Calendar if configured.",
        "input_schema": {
            "type": "object",
            "properties": {
    ...
    },
    {
        ...
        "description": "Get recent check-in notes. Use when clinician asks 'what did we discuss last time?' or 'last check-in notes'.",
        "input_schema": {
            "type": "object",
            "properties": {
...
HEVY_WRITE_TOOLS = [
    {
        ...
        "description": "Push a training routine to the patient's Hevy app. The routine will appear in their app ready to use. Specify exercises with sets, reps, and weight targets.",
        "input_schema": {
            "type": "object",
            "properties": {
...
CLINICIAN_TOOLS = CLINICIAN_CDE_TOOLS + CLINICAL_SURFACE_TOOLS + HEVY_TOOLS + HEVY_WRITE_TOOLS + CHECKIN_TOOLS
PATIENT_TOOLS = TOOL_DEFINITIONS + HEVY_TOOLS  # All CDE tools + workout tools

BASE_RULES = """You are the Clinic Platform assistant — a health data intelligence system for hormone optimization patients and their clinicians.
...
CLINICIAN_ADDENDUM = """
ROLE: You are a clinical decision support assistant for a TRT/hormone optimization clinician.
- You can READ all patient health data: labs, wearables, prescribed medications, findings, calendar.
- You can MANAGE the patient's full protocol: medications, training guidance, nutrition, recovery, and scheduling.
- When prescribing or modifying medications: call check_compound_active first, confirm dose/frequency/route, then log the event.
- When presenting drug levels, note they are estimated from the logged protocol, not measured serum levels.
- Use clinical terminology. Reference relevant lab markers when discussing medication decisions.
- Flag potential drug interactions or contraindications when you detect them.
"""

PATIENT_ADDENDUM = """
ROLE: You are assisting a patient at a hormone optimization clinic with their health data.
- You can read all your health data and log your own medication usage.
- For medication updates: always call check_compound_active first. Confirm details before logging.
- Direct clinical questions to your clinician.
"""
...
def _execute_tool_for_role(
    ...
    # Clinician-only clinical surface tools
    if tool_name == "set_training_block":
        op = SetTrainingBlock(patient_id=user_id, **tool_input)
        result = commit_operation(conn, clinician_id or "unknown", "clinician", op)
        from clinic.hevy import create_notification
        create_notification(conn, user_id, clinician_id, "clinician", "training_updated", f"Your clinician set a new training block: {tool_input.get('name', '')}", result["rendered"], None)
        return f"Committed: {result['rendered']}"
    if tool_name == "set_nutrition_target":
        op = SetNutritionTarget(patient_id=user_id, **tool_input)
        result = commit_operation(conn, clinician_id or "unknown", "clinician", op)
        from clinic.hevy import create_notification
        create_notification(conn, user_id, clinician_id, "clinician", "nutrition_updated", f"Your clinician updated your nutrition targets", result["rendered"], None)
        return f"Committed: {result['rendered']}"
    if tool_name == "add_recovery_note":
        op = AddRecoveryNote(patient_id=user_id, **tool_input)
        result = commit_operation(conn, clinician_id or "unknown", "clinician", op)
        from clinic.hevy import create_notification
        create_notification(conn, user_id, clinician_id, "clinician", "recovery_note", f"Your clinician added a recovery note", result["rendered"], None)
        return f"Committed: {result['rendered']}"

    # Clinician substance modifications — notify patient
    if role == "clinician" and tool_name == "add_compound_event":
        ...
        rendered = f"Clinician {event_type.lower()} {compound}{f' {dose}mg' if dose else ''}"
        create_notification(conn, user_id, clinician_id, "clinician",
                            "substance_modified", f"Your clinician modified your protocol: {rendered}",
                            f"{rendered}. Please review and confirm.", _json.dumps(tool_input))
        return result

    if role == "clinician" and tool_name == "update_treatment_status":
        result = execute_tool(conn, tool_name, tool_input, user_id=user_id)
        from clinic.hevy import create_notification
        new_status = tool_input.get("new_status", "unknown")
        create_notification(conn, user_id, clinician_id, "clinician",
                            "treatment_status_changed", f"Your treatment status was updated to {new_status}",
                            f"Treatment status changed to {new_status}. Please review.", None)
        return result

    # Clinician clinical-surface writes — also notify patient
    if role == "clinician" and tool_name in ("set_training_block", "set_nutrition_target", "add_recovery_note"):
        pass  # handled above with commit_operation, notification handled at API level

    # Check-in tools
    if tool_name == "schedule_checkin":
        from clinic.database import get_db as _get_db
        _conn = conn
        _conn.execute("""
        ...
        """, (user_id, tool_input.get("date"), tool_input.get("time", "12:00"), tool_input.get("description"), clinician_id, clinician_id))
        _conn.commit()
        from clinic.hevy import create_notification
        create_notification(_conn, user_id, clinician_id, "clinician", "checkin_scheduled",
                            f"Check-in scheduled for {tool_input.get('date')} at {tool_input.get('time', '12:00')}", tool_input.get("description"), None)
        return f"Check-in scheduled for {tool_input.get('date')} at {tool_input.get('time', '12:00')}. Patient notified."
    ...
    if tool_name == "add_checkin_notes":
        conn.execute("INSERT INTO check_in_note (scheduled_event_id, clinician_id, patient_id, notes_text) VALUES (?, ?, ?, ?)",
                     (tool_input["event_id"], clinician_id or "unknown", user_id, tool_input["notes_text"]))
    ...
    # Hevy routine push (clinician only)
    if tool_name == "push_routine_to_hevy":
        from clinic.hevy import push_routine_to_hevy
        result = push_routine_to_hevy(conn, clinician_id or "unknown", user_id, tool_input["title"], tool_input["exercises"])
        return f"Routine '{result['title']}' pushed to patient's Hevy ({result['exercise_count']} exercises). Patient has been notified."

    # Hevy workout tools
    if tool_name == "get_workout_history":
        from clinic.hevy import get_workout_history
    ...
    if tool_name == "get_training_summary":
        from clinic.hevy import get_training_summary
        ...
        for block in response.content:
            if block.type == "tool_use":
                result = _execute_tool_for_role(
                    ...
                    user_id=user_id, role=role, clinician_id=clinician_id,
                )
                tool_results.append({
                    "type": "tool_result",
...
def chat_clinician(
    ...
    """Clinician chat — reads patient data, manages medications/training/nutrition."""
    # Get patient name for context
    pt = conn.execute("SELECT name FROM patient WHERE id = ?", (patient_id,)).fetchone()
    patient_name = pt["name"] if pt else patient_id

    briefing = build_minimal_briefing(conn, user_id=patient_id)

    system = BASE_RULES + CLINICIAN_ADDENDUM + f"\nPATIENT: {patient_name}\n\n" + briefing

    messages = list(history or [])
    messages.append({"role": "user", "content": message})

    return _run_tool_loop(conn, system, messages, CLINICIAN_TOOLS,
                          user_id=patient_id, role="clinician", clinician_id=clinician_id)
...
def chat_patient(
    ...
    """Patient chat — full CDE experience, all 20 tools."""
    briefing = build_minimal_briefing(conn, user_id=patient_id)

    system = BASE_RULES + PATIENT_ADDENDUM + "\n\n" + briefing

    messages = list(history or [])
    messages.append({"role": "user", "content": message})

    return _run_tool_loop(conn, system, messages, PATIENT_TOOLS,
