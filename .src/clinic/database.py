"""Multi-tenant database for Clinic Platform.
...
without modification. The key column: `user_id` stores the patient's ID.

Clinic Platform-specific tables (clinician, patient, clinician_patient, training_block,
nutrition_target, recovery_note, operation_log, integration_status) use
`patient_id` and `clinician_id` for multi-tenant clarity.
"""
import sqlite3
import os
from clinic.config import CLINIC_DB_PATH
...
def init_db(conn: sqlite3.Connection):
    ...
    -- ══════════════════════════════════════════════════════════
    -- Clinic Platform-specific tables (multi-tenant identity)
    -- ══════════════════════════════════════════════════════════
    ...
    CREATE TABLE IF NOT EXISTS clinician_patient (
    ...
    -- ══════════════════════════════════════════════════════════
    -- CDE-compatible data tables (user_id = patient.id)
    ...
    -- ══════════════════════════════════════════════════════════
    -- Clinic Platform clinical-surface tables
    -- ══════════════════════════════════════════════════════════
    ...
    CREATE INDEX IF NOT EXISTS idx_training_patient ON training_block(patient_id);
    CREATE INDEX IF NOT EXISTS idx_nutrition_patient ON nutrition_target(patient_id);
    ...
    CREATE INDEX IF NOT EXISTS idx_checkin_note_patient ON check_in_note(patient_id, created_at);
    ...
    CREATE TABLE IF NOT EXISTS onboarding_file (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        session_id TEXT NOT NULL REFERENCES onboarding_session(id),
    ...
    CREATE TABLE IF NOT EXISTS onboarding_conflict (
        id INTEGER PRIMARY KEY AUTOINCREMENT,
        file_id INTEGER NOT NULL REFERENCES onboarding_file(id),
    ...
    CREATE INDEX IF NOT EXISTS idx_onboard_session_clinician ON onboarding_session(clinician_id);
    CREATE INDEX IF NOT EXISTS idx_onboard_file_session ON onboarding_file(session_id);
    CREATE INDEX IF NOT EXISTS idx_onboard_conflict_file ON onboarding_conflict(file_id);
    """)
    conn.commit()
