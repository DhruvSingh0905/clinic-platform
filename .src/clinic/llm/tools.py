...
from clinic.compound_db import lookup_compound as _py_lookup_compound
...
def get_all_active_compounds(conn: sqlite3.Connection, user_id: str = "default") -> str:
    ...
    Returns current dose, frequency, route, and level for each. Used to
    enumerate every compound in the patient's protocol.
    """
...
def update_treatment_status(
    ...
    """Update treatment status. Closes the prior status and opens the new one.
    ...
    valid_statuses = ("active_treatment", "monitoring", "tapering", "discontinued")
    ...
    if current:
        if current["phase"] == new_status:
            return f"Already in {new_status} status (since {current['started_at'][:10]}). No change needed."
        conn.execute(
    ...
    conn.execute(
        ...
        (user_id, new_status, started_at, notes),
    )
    conn.commit()
    ...
    # Build confirmation
    result = f"Treatment status updated to {new_status.upper()}"
    if current:
...
def get_treatment_history(conn: sqlite3.Connection, user_id: str = "default") -> str:
    """Get the full treatment status history with dates and durations."""
    from datetime import date as date_type
    ...
    if not rows:
        return "No treatment history recorded. Treatment tracking has not been set up yet."

    lines = ["**Treatment History**\n"]
...
TREATMENT_TOOLS = [
    {
        ...
        "description": "Get ALL active compounds including ancillaries, supplements, and support medications with current dose/frequency/route. Use to enumerate every medication in the patient's protocol.",
        "input_schema": {
            "type": "object",
            "properties": {}
        }
    },
    {
        ...
        "description": "Update treatment status (active_treatment/monitoring/tapering/discontinued). Call this AFTER all compound events (STOP, DOSE_CHANGE) have been logged. Regenerates drug levels automatically.",
        "input_schema": {
            "type": "object",
            "properties": {
                "new_status": {"type": "string", "enum": ["active_treatment", "monitoring", "tapering", "discontinued"], "description": "The new treatment status"},
                "started_at": {"type": "string", "description": "ISO date when the status started (default: today)"},
                "notes": {"type": "string", "description": "Optional clinical context about this status change"}
            },
            "required": ["new_status"]
        }
    },
    {
        ...
        "description": "Get the patient's treatment status history with dates and durations. Use when asked 'how long has the patient been on treatment?', 'when did we start monitoring?', or 'what is the current treatment status?'",
        "input_schema": {
            "type": "object",
            "properties": {}
        }
...
TOOL_DEFINITIONS.extend(TREATMENT_TOOLS)
...
def execute_tool(conn: sqlite3.Connection, tool_name: str, tool_input: dict, user_id: str = "default") -> str:
    ...
    dispatch = {
        ...
        "update_treatment_status": lambda: update_treatment_status(
            conn, tool_input["new_status"],
            tool_input.get("started_at"), tool_input.get("notes"), user_id,
        ),
        "get_treatment_history": lambda: get_treatment_history(conn, user_id),
    }
