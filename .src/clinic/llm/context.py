...
def _build_treatment_section(conn: sqlite3.Connection, user_id: str) -> str:
    ...
    if row:
        ...
        return f"CURRENT TREATMENT STATUS: {row['phase'].upper()} (since {row['started_at'][:10]}, day {days})"
    return "CURRENT TREATMENT STATUS: Unknown (not set)"
...
def build_briefing(conn: sqlite3.Connection, user_id: str = "default") -> str:
    ...
    sections.append(_build_treatment_section(conn, user_id))
        ...
        if finding["detail"]:
            ...
            if detail.get("signals"):
                ...
                for s in detail["signals"]:
                    label = s.get("description") or s.get("label", "unknown")
                    current = s.get("current") or s.get("value", "")
...
def build_minimal_briefing(conn: sqlite3.Connection, user_id: str = "default") -> str:
    """Free chat briefing: treatment status + compounds + snapshot only. NO findings injected."""
    sections = [
        _build_treatment_section(conn, user_id),
