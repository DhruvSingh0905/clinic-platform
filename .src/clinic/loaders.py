...
from clinic.extraction.schema import ValidatedExtraction
from clinic.extraction.reference_data import TEST_DEFINITIONS
from clinic.apple_health import AppleHealthExport
from clinic.cycle_log import CompoundEvent
from clinic.compound_db import COMPOUNDS
from clinic.bp_entry import BPReading, classify_bp
...
def load_apple_health(
    ...
    import json as _json
    cursor = conn.execute(
        """INSERT INTO source_document (user_id, source_type, raw_storage_path, metadata_json)
           VALUES (?, 'HEALTH_XML', ?, ?)""",
        (user_id, "", _json.dumps({"source_file": source_filename})),
    )
    source_id = cursor.lastrowid
        ...
        conn.execute(
            """INSERT OR REPLACE INTO wearable_observation
               (user_id, metric, observation_date, value_mean, value_min, value_max,
                reading_count, unit, source, source_document_id)
               VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'apple_health', ?)""",
